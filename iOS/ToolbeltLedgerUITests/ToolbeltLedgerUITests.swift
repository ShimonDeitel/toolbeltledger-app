...
final class ToolbeltLedgerUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() {
        for i in 0..<(Self.freeLimitForUITest + 1) {
            ...
            if nameField.waitForExistence(timeout: 2) {
                ...
                app.buttons["entrySaveButton"].tap()
            } else if app.staticTexts["Toolbelt Ledger Pro"].waitForExistence(timeout: 2) {
                break
            }
        }
        XCTAssertTrue(app.staticTexts["Toolbelt Ledger Pro"].waitForExistence(timeout: 3))
    }
